function [s] = optimize_below_quality(model_name, mue, lambda, max_time, delta, n_gens, accepted_quality)
    ...
    while (current_gen_index < n_gens)
        ...
        % Get best quality of current generation
        best_quality = population{1}.quality;
        fprintf('Best solution in generation %d: %.4f m/s\n', ...
            current_gen_index, best_quality);
        
        % Check if quality criterion is met
        if best_quality <= accepted_quality
            fprintf('Accepted quality %.4f m/s reached in generation %d\n', ...
                accepted_quality, current_gen_index);
            break;
        end
